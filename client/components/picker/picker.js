...
class Picker extends Component {
    ...
    renderActions(){
        ...
        let elActions = this.state.actions.map( (action, i)=> {
            ...
            return <a {...others} key={i} className="msbu-picker__action"> { label }</a>
        })

        return (
            <div className="msbu-picker__hd">
                { elActions }
            </div>
        )
    }
    ...
    render(){
        ...
        const cls = classNames('msbu-picker', {
            'msbu-animate-slide-up': show && !this.state.closing,
            'msbu-animate-slide-down': this.state.closing
        }, className);

        const maskCls = classNames({
            'msbu-animate-fade-in': show && !this.state.closing,
            'msbu-animate-fade-out': this.state.closing
        })
